...
1. Run `./install -d <chroot> -m <mirror> -i <image name> -p <proper name> -u <image docs` under Void to install Void in a given directory. This will create a tarball of the installation in your working directory (named `<image name>-<YYMMDD>.tar.gz`). See `./install -h` for detailed usage.
